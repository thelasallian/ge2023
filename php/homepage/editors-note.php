<div class="py-5 px-4">
    <div class="container d-flex flex-column align-items-center editor-bg-color">
        <h1 data-aos="zoom-in" class="pt-5">Editor's Note</h1>
        <div data-aos="zoom-in"  class="container note pt-2 pb-5">
            <p class="mb-0">Lorem ipsum dolor sit amet, consectetur adipisici elit, sed eiusmod tempor incidunt ut labore et dolore magna aliqua. Donec sed odio operae, eu vulputate felis rhoncus. Salutantibus vitae elit libero, a pharetra augue. Nihil hic munitissimus habendi senatus locus, nihil horum? A communi observantia non est recedendum.</p>
        </div>
    </div>
</div>